...
// Media group buffer interface
interface MediaGroupBuffer {
  messages: IMMessage[];
  ctx: Context;  // Save the first message's ctx for replying
...
export class TelegramGateway extends EventEmitter {
  ...
  // Media group buffer Map (mediaGroupId -> buffer)
  private mediaGroupBuffers: Map<string, MediaGroupBuffer> = new Map();
  private readonly MEDIA_GROUP_TIMEOUT = 500;  // 500ms buffer window
  ...
  async start(config: TelegramConfig): Promise<void> {
    ...
    try {
      ...
      // Set up periodic cleanup task (every 24 hours)
  ...
  async stop(): Promise<void> {
    ...
    try {
      // Clean up periodic task
      if (this.cleanupInterval) {
        clearInterval(this.cleanupInterval);
        this.cleanupInterval = null;
      }

      // Clean up media group buffers
  ...
  private async handleMessage(ctx: Context): Promise<void> {
    try {
      ...
      console.log(`[Telegram] Received message:`, JSON.stringify({
        sender: senderName,
        senderId,
        chatId,
  ...
  private async processMessage(imMessage: IMMessage, ctx: Context): Promise<void> {
    ...
    const replyFn = async (text: string) => {
      ...
      console.log(`[Telegram] Sending reply:`, JSON.stringify({
      ...
      try {
        ...
        log(`[Telegram Gateway] Parsed media markers:`, JSON.stringify({
        ...
        for (const marker of markers) {
          ...
          if (fs.existsSync(filePath)) {
            ...
            log(`[Telegram Gateway] Found valid media file:`, JSON.stringify({
              path: filePath,
              name: marker.name,
              type: marker.type,
              fileSize: stats.size,
        ...
        // Note: keep original markdown text, don't remove media markers
        ...
        for (const file of validFiles) {
          const sendMedia = async (): Promise<boolean> => {
            // Use Buffer instead of file path to avoid node-fetch streaming issues
            ...
            // Full URL (not sanitized, for debugging)
          ...
          for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
              ...
              // Print detailed request info before each attempt (showing full URL)
              ...
              // Print detailed failure info (showing full URL)
