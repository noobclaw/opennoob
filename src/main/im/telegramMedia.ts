...
function generateFileName(fileId: string, extension: string): string {
  ...
  const shortId = fileId.slice(-8);  // Take last 8 chars of file_id
...
export async function extractMediaFromMessage(
  ...
  // 1. Photo - take highest resolution
  if (msg.photo && msg.photo.length > 0) {
    const photo = msg.photo[msg.photo.length - 1];  // Last one is the largest
  ...
  // 7. Stickers (static WEBP only)
  if (msg.sticker) {
    const sticker = msg.sticker;
    // Only process static stickers, skip animated stickers (TGS) and video stickers (WEBM)
