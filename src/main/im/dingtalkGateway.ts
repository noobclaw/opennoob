...
export class DingTalkGateway extends EventEmitter {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    if (accessToken && accessTokenExpiry > now + 60000) {
      this.log('[DingTalk Gateway] Using cached AccessToken');
      return accessToken;
    }

    this.log('[DingTalk Gateway] Fetching new AccessToken...');
    ...
    this.log(`[DingTalk Gateway] AccessToken obtained successfully, expires at: ${new Date(accessTokenExpiry).toLocaleString()}`);
    return accessToken;
  }
    ...
    for (let attempt = 1; attempt <= this.MAX_RETRIES; attempt++) {
      try {
        ...
        if (status === 401 || status === 403) {
          accessToken = null;
          accessTokenExpiry = 0;
          this.log(`[DingTalk Gateway] Token may have expired, cache cleared`);
        }
        if (attempt === this.MAX_RETRIES) {
          console.error(`[DingTalk Gateway] ${label} finally failed (after ${this.MAX_RETRIES} attempts): ${error.message}`);
          throw error;
        }
        console.warn(`[DingTalk Gateway] ${label} failed (${attempt}/${this.MAX_RETRIES}): ${error.message}, retrying in ${this.RETRY_DELAY / 1000}s...`);
  ...
  private async sendBySession(
    ...
    let body: any;
    if (useMarkdown) {
      const title = text.split('\n')[0].replace(/^[#*\s\->]+/, '').slice(0, 20) || 'Assistant';
      let finalText = text;
    ...
    this.log(`[DingTalk] Sending text message:`, JSON.stringify({
  ...
  private async sendMediaViaNewApi(
    ...
    // msgParam must be a JSON string
    ...
    this.log(`[DingTalk] Sending media message:`, JSON.stringify({
      msgKey,
      msgParam,
    ...
    await this.retryableRequest(async () => {
      ...
      // Check response (new API error format may differ)
  ...
  private async sendWithMedia(
    ...
    this.log(`[DingTalk Gateway] Parsed media markers:`, JSON.stringify({
      textLength: text.length,
    ...
    if (markers.length === 0) {
      // No media, send text directly
      await this.sendBySession(sessionWebhook, text, options);
      return;
    }

    // Get oapi token (for media upload, different from new API token)
    if (!this.config) {
      throw new Error('DingTalk config not set');
    }
    ...
    for (const marker of markers) {
      ...
      this.log(`[DingTalk Gateway] Uploading media file:`, JSON.stringify({
        path: marker.path,
        name: marker.name,
        type: marker.type,
        mediaType,
      ...
      this.log(`[DingTalk Gateway] Media upload successful:`, JSON.stringify({
      ...
      try {
        ...
        // Use the new API to send media message
    ...
    // Send the full original text (keep markdown format, don't remove media markers)
    await this.sendBySession(sessionWebhook, text, options);
  }
  ...
  private buildMediaMessage(mediaType: string, mediaId: string, fileName?: string): DingTalkMediaMessage {
    switch (mediaType) {
      ...
      default:
        // File type supports custom file name
  ...
  private async handleInboundMessage(data: DingTalkInboundMessage): Promise<void> {
    ...
    this.log(`[DingTalk] Received message:`, JSON.stringify({
      sender: senderName,
      senderId,
    ...
    const replyFn = async (text: string) => {
      ...
      this.log(`[DingTalk] Sending reply:`, JSON.stringify({
        conversationId: data.conversationId,
