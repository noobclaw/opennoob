...
export class DiscordGateway extends EventEmitter {
  ...
  async start(config: DiscordConfig): Promise<void> {
    ...
    try {
      ...
      this.log('[Discord Gateway] Creating Client instance, intents: Guilds, GuildMessages, DirectMessages, MessageContent');
      ...
      this.log('[Discord Gateway] Registering event handlers: Error, ClientReady, MessageCreate');
      ...
      this.log('[Discord Gateway] Logging in Bot...');
      await this.client.login(config.botToken);
      this.log('[Discord Gateway] Login request sent, waiting for ClientReady event...');
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      this.log('[Discord Gateway] Destroying Client connection...');
      client.destroy();
  ...
  private async handleMessage(message: Message): Promise<void> {
    try {
      ...
      this.log(`[Discord] Received message:`, JSON.stringify({
        sender: senderName,
        senderId,
        conversationId,
      ...
      const replyFn = async (text: string) => {
        ...
        this.log(`[Discord] Sending reply:`, JSON.stringify({
          conversationId,
        ...
        try {
          ...
          this.log(`[Discord Gateway] Parsed media markers:`, JSON.stringify({
          ...
          for (const marker of markers) {
            ...
            if (fs.existsSync(filePath)) {
              ...
              this.log(`[Discord Gateway] Found valid media file:`, JSON.stringify({
                path: filePath,
                name: marker.name,
                type: marker.type,
                fileSize: stats.size,
          ...
          this.log(`[Discord Gateway] Preparing to send:`, JSON.stringify({
          ...
          if (textContent.length <= MAX_LENGTH) {
            ...
            if (attachments.length > 0) {
              ...
              this.log(`[Discord Gateway] Sent text+attachment message`);
            } else if (textContent) {
              await message.reply(textContent);
              this.log(`[Discord Gateway] Sent plain text message`);
            }
          } else {
            // Split by newlines or by length
            const chunks = this.splitMessage(textContent, MAX_LENGTH);
            this.log(`[Discord Gateway] Message too long, split into ${chunks.length} parts`);
            ...
            this.log(`[Discord Gateway] Sent all ${chunks.length} messages`);
          }
  ...
  async sendNotification(text: string): Promise<void> {
    ...
    this.log(`[Discord Gateway] Sending notification message:`, JSON.stringify({
    ...
    if (channel && channel.isTextBased() && 'send' in channel) {
      ...
      this.log(`[Discord Gateway] Notification message sent`);
      this.status.lastOutboundAt = Date.now();
    } else {
