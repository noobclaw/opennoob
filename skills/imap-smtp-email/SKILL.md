...
name_zh: "邮件管理"
description_zh: "通过 IMAP/SMTP 协议收发和管理邮件"
official: true
version: 1.0.1
---
...
The `.env` configuration file is automatically managed by OpenNoob Settings (邮箱设置). **Do NOT ask the user to create or edit `.env` — just run the commands directly.** If credentials are wrong, the scripts will return a clear error message; only then should you inform the user to check their email settings.
