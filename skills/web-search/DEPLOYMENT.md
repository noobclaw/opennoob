...
export async function getEnhancedEnv(): Promise<Record<string, string | undefined>> {
  ...
  env.OPENNOOB_SKILLS_ROOT = skillsRoot; // Alternative name for clarity
...
2. **Production Mode**:
   - `SKILLS_ROOT` → `~/Library/Application Support/opennoob/SKILLs` (macOS)
...
### Related Files (in the host app)

The skill expects the host application to provide:

- An environment-variable injection point for `SKILLS_ROOT` (typically the
  agent runner that spawns the skill subprocess)
- A skill-directory manager that copies bundled skills into userData on
  first launch and exposes their resolved path
- A background-service supervisor for the long-running `start-server.sh`

See `SKILL.md` for the skill's runtime contract.
