...
resolve_node_runtime() {
  ...
  if [ -n "${OPENNOOB_ELECTRON_PATH:-}" ] && [ -x "${OPENNOOB_ELECTRON_PATH}" ]; then
    NODE_CMD="$OPENNOOB_ELECTRON_PATH"
    NODE_ARGS=()
...
ensure_npm_available() {
  ...
  echo "  Please reinstall the web-search skill runtime from OpenNoob."
  return 1
}
...
if ! resolve_node_runtime; then
  ...
  echo "  Please install Node.js, or run from OpenNoob so scripts can use Electron runtime."
  exit 1
fi
