 ...
 * - Offline archive support via OPENNOOB_PORTABLE_GIT_ARCHIVE
 * - Mirror URL override via OPENNOOB_PORTABLE_GIT_URL
...
async function resolveArchive(required) {
  const envArchive = resolveInputPath(process.env.OPENNOOB_PORTABLE_GIT_ARCHIVE);
  if (envArchive) {
    if (!isNonEmptyFile(envArchive)) {
      throw new Error(
        `OPENNOOB_PORTABLE_GIT_ARCHIVE points to an invalid file: ${envArchive}`
      );
    }
    console.log(`[setup-mingit] Using local archive from OPENNOOB_PORTABLE_GIT_ARCHIVE: ${envArchive}`);
  ...
  const urlFromEnv = typeof process.env.OPENNOOB_PORTABLE_GIT_URL === 'string'
    ? process.env.OPENNOOB_PORTABLE_GIT_URL.trim()
    : '';
  ...
  try {
    ...
    if (required) {
      throw new Error(
        ...
        + 'Set OPENNOOB_PORTABLE_GIT_ARCHIVE to a local offline package or '
        + 'set OPENNOOB_PORTABLE_GIT_URL to a reachable mirror. '
        + `Original error: ${error instanceof Error ? error.message : String(error)}`
      );
    }
...
async function ensurePortableGit(options = {}) {
  ...
  const shouldRun = process.platform === 'win32' || required || process.env.OPENNOOB_SETUP_MINGIT_FORCE === '1';
